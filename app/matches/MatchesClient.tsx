...
import { GoalToast } from "@/components/GoalToast/GoalToast";
import { useLiveMatches, type GoalEvent } from "@/hooks/useLiveMatches";
...
export default function MatchesClient({ initialMatches, initialError }: Props) {
  ...
  // If server pre-fetched data, start with no loading spinner
  ...
  // ── Phase 1: Live SSE connection ──────────────────────────────────────────
  const [lastGoalEvent, setLastGoalEvent] = useState<GoalEvent | null>(null);
  const { matches: liveMatches, isConnected, isLoading: liveLoading, lastUpdated, reconnect } =
    useLiveMatches({
      date: selectedDate,
      onGoal: (event) => setLastGoalEvent(event),
      // Only connect on today — past/future dates don't have live scores
      disabled: selectedDate !== todayIso(),
    });

  // Merge live score updates into the fetched matches list
  useEffect(() => {
    if (!liveMatches.length) return;
    setMatches(prev => {
      const liveMap = new Map(liveMatches.map(m => [m.id, m]));
      return prev.map(m => liveMap.get(m.id) ?? m);
    });
  }, [liveMatches]);
  ...
  // ── Fetch ─────────────────────────────────────────────────────────────────
  // Skip the first fetch if the server already pre-fetched matches for today.
  // This makes the page instant on first load — no spinner, no wait.
  const initialDateRef = useRef(selectedDate);
  const hasInitialData = useRef(initialMatches.length > 0);

  useEffect(() => {
    // Skip first fetch if server pre-fetched today's data
    const isFirstFetchForToday = selectedDate === initialDateRef.current && hasInitialData.current;
    if (isFirstFetchForToday) {
  ...
  return (
    <div className={styles.shell}>
      ...
      <div className={styles.main}>
        ...
        {!loading && matches.length > 0 && (
          <div className={styles.countBar}>
            <span className={styles.countLabel}>
              {isFallback ? "Upcoming fixtures (demo)" : isoLabel(selectedDate)}
            </span>
        ...
        {/* Empty state */}
        {!loading && !fetchError && groups.length === 0 && liveOnly && (
          <div className={styles.empty}>
            <div className={styles.emptyIcon}>📺</div>
            <h2 className={styles.emptyTitle}>No live matches right now</h2>
            <p className={styles.emptyText}>No games are currently in play. Check back soon.</p>
      ...
      {/* Phase 1: in-app goal toast */}
      <GoalToast event={lastGoalEvent} />
